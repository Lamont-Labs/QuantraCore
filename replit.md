...
- **Offline ML (ApexCore v3):** On-device neural models with 5 prediction heads, incorporating calibrated probabilities, regime-gated ensemble routing, uncertainty quantification, protocol telemetry weighting, and cross-asset context integration for reliable confidence scores and prediction intervals.
...
- **Alpaca Markets API:** Primary data source (200 requests/minute) for OHLCV and quotes, and for paper trading execution.
- **Polygon.io:** Backup data source (5 requests/minute) for OHLCV and options data.
