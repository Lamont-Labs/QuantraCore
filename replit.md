...
- **Offline ML (ApexCore v3):** On-device neural models with 6 prediction heads (quantrascore, runner, quality, avoid, regime, **timing**), trained on real market data with actual outcome labels. The **timing head** predicts WHEN a stock will start its significant move using 5 timing buckets: immediate (1 bar), very_soon (2-3 bars), soon (4-6 bars), late (7-10 bars), none. Current model trained on **107,978 samples** from **59 symbols** using **222,412 15-minute intraday bars** (2 years of data) via Alpaca IEX feed, achieving **98.88% runner accuracy**, **93.75% quality accuracy**, **99.91% avoid accuracy**, and **95.25% regime accuracy**. QuantraScore RMSE: 0.00999.
